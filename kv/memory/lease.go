...
func (m *MemoryKV) Acquire(lease []byte, ttl time.Duration) (token uint64, err error) {
...
func (m *MemoryKV) Renew(lease []byte, ttl time.Duration, prevToken uint64) (newToken uint64, err error) {
	ttl, ok := durationGuard(ttl)
	if !ok {
		return 0, chord.ErrKVLeaseInvalidTTL
	}
...
func (m *MemoryKV) Release(lease []byte, token uint64) error {
	v, _ := m.fetchVal(lease)
