...
func (m *Connection) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.CacheDirection != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.CacheDirection))
		i--
		dAtA[i] = 0x18
	}
	if m.CacheState != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.CacheState))
		i--
		dAtA[i] = 0x10
	}
	if m.Identity != nil {
...
func (m *Connection) SizeVT() (n int) {
	...
	if m.CacheState != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.CacheState))
	}
	if m.CacheDirection != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.CacheDirection))
	}
	n += len(m.unknownFields)
	return n
}
