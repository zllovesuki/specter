...
func (m *RenewalRequest) MarshalVT() (dAtA []byte, err error) {
	if m == nil {
		return nil, nil
	}
	size := m.SizeVT()
...
func (m *RenewalRequest) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *RenewalRequest) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	if m == nil {
		return 0, nil
	}
	i := len(dAtA)
	...
	if len(m.PrevCertDer) > 0 {
		i -= len(m.PrevCertDer)
		copy(dAtA[i:], m.PrevCertDer)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.PrevCertDer)))
		i--
		dAtA[i] = 0x12
	}
	if m.Proof != nil {
...
func (m *RenewalRequest) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
...
func (m *RenewalRequest) UnmarshalVT(dAtA []byte) error {
	l := len(dAtA)
	iNdEx := 0
	for iNdEx < l {
		preIndex := iNdEx
		...
		wireType := int(wire & 0x7)
		if wireType == 4 {
			return fmt.Errorf("proto: RenewalRequest: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: RenewalRequest: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field PrevCertDer", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.PrevCertDer = append(m.PrevCertDer[:0], dAtA[iNdEx:postIndex]...)
			if m.PrevCertDer == nil {
				m.PrevCertDer = []byte{}
			}
			iNdEx = postIndex
		default:
