...
func (m *LeaseRequest) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.PrevToken != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.PrevToken))
		i--
		dAtA[i] = 0x18
	}
	if m.NewToken != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.NewToken))
		i--
		dAtA[i] = 0x10
	}
	if len(m.Lease) > 0 {
		i -= len(m.Lease)
		copy(dAtA[i:], m.Lease)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.Lease)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
...
func (m *KVTransfer) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.LeaseToken != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.LeaseToken))
		i--
		dAtA[i] = 0x18
	}
	if len(m.PrefixChildren) > 0 {
		for iNdEx := len(m.PrefixChildren) - 1; iNdEx >= 0; iNdEx-- {
			...
			i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.PrefixChildren[iNdEx])))
			i--
			dAtA[i] = 0x12
		}
	}
...
func (m *LeaseRequest) SizeVT() (n int) {
	...
	if m.NewToken != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.NewToken))
	}
	if m.PrevToken != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.PrevToken))
	}
	if m.Ttl != nil {
...
func (m *KVTransfer) SizeVT() (n int) {
	...
	if m.LeaseToken != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.LeaseToken))
	}
	n += len(m.unknownFields)
	return n
}
